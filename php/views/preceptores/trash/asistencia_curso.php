...
<body>
    ...
    <?php
        ...
        while ($row = $r_query_all_curso->fetch_assoc()){
            ?>
            <a href="asistencia_curso.php?id_curso=<?php echo $row['id']; ?>" class="none_cursos btn btn-secondary"><?php echo $row['anio'], "° ", $row['division'], "°";?></a>
            <?php
        }
        ?>
        </div>
        ...
        if (!empty($_GET['id_curso'])){
            include "asistencia_materias.php";
